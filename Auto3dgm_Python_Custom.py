...
class interface:
    ...
    def saveSettings(
        ...
        if len(mesh_dir) == 0:
            tk.messagebox.showerror(
                "Error", "You need to select your mesh directory!", icon="error"
            )
            return
        if len(output_dir) == 0:
            tk.messagebox.showerror(
                "Error", "You need to select your output directory!", icon="error"
            )
            return
    ...
    def setSettings(self):
        ...
        tk.Label(self.root, text="Number of Subsampled Points").grid(
            row=0, columnspan=SPAN_WIDTH, sticky="W"
        )
        ...
        mesh_dir_entry, output_dir_entry = (
            tk.Entry(self.root, width=FILE_WIDTH),
            tk.Entry(self.root, width=FILE_WIDTH),
        )

        # Toggle Button
        # Weird work around for scope
        reflection_btn = tk.Button(
            self.root, text="Reflection", width=20, command=lambda: self.switchRefl()
        )

        self.reflection_btn = reflection_btn

        # Radio Buttons
        num_cores_r1 = tk.Radiobutton(
            self.root, text="Single Core", value=1, var=num_cores_var
        )
        num_cores_r2 = tk.Radiobutton(
            self.root, text="Multiple Cores", value=0, var=num_cores_var
        )

        sample_method_r1 = tk.Radiobutton(
            self.root, text="FPS", value="FPS", var=sample_method_var
        )
        sample_method_r2 = tk.Radiobutton(
            self.root, text="GPL", value="GPL", var=sample_method_var
        )
        sample_method_r3 = tk.Radiobutton(
            self.root, text="Hybrid", value="Hybrid", var=sample_method_var
        )
        sample_method_r2["state"] = "disabled"
        sample_method_r3["state"] = "disabled"

        # Set up the browse and save buttons
        btn_bws_mesh = tk.Button(
            self.root,
            height=1,
            width=10,
            text="Browse",
    ...
    def visualize(self):
        ...
        daemon = threading.Thread(
            name="daemon_server", target=self.start_server, args=(".", PORT)
        )
        daemon.setDaemon(
            True
    ...
    def complete(self):
        ...
        btn_yes = tk.Button(
            self.root, height=1, width=10, text="Yes", command=lambda: self.visualize()
        )
        ...
        btn_yes.grid(
            row=1, column=0, sticky="W", columnspan=SPAN_WIDTH, pady=PADY, padx=PADX
        )
        btn_no.grid(
            row=1, column=2, sticky="W", columnspan=SPAN_WIDTH, pady=PADY, padx=PADX
        )
...
if __name__ == "__main__":
    ...
    ### START OF NEW CODE CALL EXAMPLE ###
    # After the GUI closes and the alignment + landmark export has finished,
    # you can call run_auto3dgm_db_integration() as a final step:
    # (Or you can place this call somewhere inside `complete()` in the interface class,
    #  if you want it to run automatically.)
    #
    ### END OF NEW CODE CALL EXAMPLE ###
